...
Github3.prototype._request = function (options, callback) {
  ...
  options.headers['User-Agent'] = 'Node github3 v' + VERSION;
...
Github3.prototype.createBlob = function (repo, name, content, callback) {
  if (!Buffer.isBuffer(content)) {
    callback(new Error("createBlob: content is not a Buffer"), null);
    return;
  }
  ...
  return this._post('/repos/'+name+'/'+repo+'/git/blobs', JSON.stringify(blob), callback);
};
 ...
 */
Github3.prototype.createTreeWithBlob = function (repo, name, path, blob_sha, last_tree_sha, callback) {
  var new_tree = {
